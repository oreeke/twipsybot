...
管理命令保存的模型、白名单和黑名单覆盖会在重启后继续生效。需要回到配置文件值时，使用对应的 `reset` 命令。
...
`db.clear` 只按保留期限清理用户回复限制状态，包括最近回复时间、累计轮数和临时封禁期限；不会清理自动发帖计数、管理命令覆盖或插件私有数据。手动删除数据库则会重置以上全部状态，包括管理命令保存的模型、白名单和黑名单。
