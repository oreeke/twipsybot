...
Topics 在自动发帖任务触发时提供内容源。它不会创建独立定时器，运行间隔和每日上限仍由 `bot.auto_post` 控制。
...
开源软件的长期维护
城市夜间公共交通
https://example.com/an-interesting-page
```

插件按顺序循环读取非空行。普通主题通过 `{topic}` 加入自动发帖提示词；整行是纯 HTTP 或 HTTPS URL 时，会直接发布链接，不调用模型。
...
`rss_ai: false` 时发布摘要或标题和原文链接；开启后，模型根据 `{summary}`、`{title}`、`{link}` 生成一段文本，帖子仍会附带链接。RSS 至少应提供标题和链接。
