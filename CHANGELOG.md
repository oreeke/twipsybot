...
- Streaming 新增 `chatRoom` 支持，房间消息使用独立频道接收并标记已读
- 动态聊天频道使用 `pong` 回执确认订阅，并限制单个连接最多使用 32 个频道
- Misskey REST 错误保留状态码、错误代码、错误 ID、类别和附加信息

### Changed

- Plugin API 升级到 v3，移除 `misskey.create_note(..., validate_reply=...)` 参数
- 回复可见性、联合范围、目标可见性和屏蔽校验交由 Misskey 服务端处理；指定可见回复直接复用事件信息
- REST 和 Drive 上传统一使用 Bearer 认证，不再重复发送请求体或 multipart `i`
- 读取请求遇到 429 时遵循服务端 `Retry-After`，写入请求继续保持不自动重试
- 超长帖子和聊天文本在 Misskey 客户端统一安全截断
- Vision 直接使用事件中的文件类型和 URL 读取附件，不再查询发送者的 Drive 元数据

### Fixed

- 修复房间聊天错误订阅 `chatUser` 并误标一对一会话已读的问题
- 修复权限不足、资源不存在、上传过大和限流被归入错误类别的问题
- 修复清理帖子时将权限错误静默视为可跳过资源的问题
